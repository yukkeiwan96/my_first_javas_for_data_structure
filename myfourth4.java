...
/* QuickInserC reads in a line of integers from a text file which is then split into an array
 * It uses the first element as pivot
 * It has a partition size of 50, which is then followed by insertion sort
 * quickinserc sorts the array iteratively until the partition size 50.
 * each partition of size 50 is then sorted by insertion sort
 */

public class QuickInserC { 
    ...
    public void quickinserc(int arr[], int l, int h) { 
       if ((h-l)>50){ 
 ...
 public static void printArray(int nums[], FileWriter outputStream) throws IOException{ 
  ...
  public static void main (String args[]) throws IOException {
    ...
    int n;
    QuickInserC qc= new QuickInserC();
    ...
    //  Open the files that will be used for input and output, and writes the output file.
     try{
        ...
        outputStream.write("Quick Sort: partition size of 50 followed by insertion sort."+"\r\n");
        long startTimed = System.nanoTime();
        qc.quickinserc(nums, 0, le-1);
